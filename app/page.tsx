...
import { PWAInstallPrompt } from "./components/PWAManager";
import { ZaloNotifyButton } from "./components/ZaloSettings";
import { TelegramNotifyButton } from "./components/TelegramSettings";
...
✓ Đối soát chênh lệch: 0 đ (Khớp 100%)`;
  ...
  // Update PDF content for printing
  useEffect(() => {
    ...
    pdfContent.innerHTML = `
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', -apple-system, sans-serif; color: #1e293b; padding: 20px; }
        ...
        .logo { width: 48px; height: 48px; background: linear-gradient(135deg, #0d9488, #0f766e); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 24px; }
        .header-title { font-size: 18px; font-weight: 800; color: #0f172a; }
        .header-subtitle { font-size: 12px; color: #64748b; }
        .header-right { text-align: right; }
        .month-badge { display: inline-block; background: #0d9488; color: white; padding: 6px 12px; border-radius: 8px; font-size: 14px; font-weight: 700; }
        .total-evn { font-size: 11px; color: #64748b; margin-top: 4px; }
        .summary-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px; }
        .summary-item { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 14px; text-align: center; }
        .summary-label { font-size: 10px; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px; }
        .summary-value { font-size: 18px; font-weight: 800; color: #0f172a; }
        .summary-unit { font-size: 10px; color: #94a3b8; }
        .households-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px; }
        .household-card { border-radius: 14px; padding: 16px; }
        .tret-card { background: linear-gradient(135deg, rgba(45, 212, 191, 0.08), rgba(13, 148, 136, 0.05)); border: 1px solid rgba(45, 212, 191, 0.2); }
        .lau-card { background: linear-gradient(135deg, rgba(139, 92, 246, 0.08), rgba(109, 40, 217, 0.05)); border: 1px solid rgba(139, 92, 246, 0.2); }
        .household-header { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 8px; }
        .tret-header { color: #0d9488; }
        .lau-header { color: #7c3aed; }
        .household-amount { font-size: 28px; font-weight: 900; margin-bottom: 12px; }
        .tret-amount { color: #0d9488; }
        .lau-amount { color: #7c3aed; }
        .household-details { padding-top: 10px; border-top: 1px solid rgba(0,0,0,0.06); }
        ...
        .tret-value { color: #0d9488; }
        .lau-value { color: #7c3aed; }
        .verification-bar { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 10px; padding: 12px; margin-bottom: 20px; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .verification-icon { color: #22c55e; font-size: 16px; }
        .verification-text { font-size: 12px; font-weight: 600; color: #166534; }
        .section { margin-bottom: 20px; }
        .section-title { font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 10px; padding-bottom: 6px; border-bottom: 1px solid #e2e8f0; }
        .tier-table { width: 100%; border-collapse: collapse; font-size: 11px; }
        .tier-table th { text-align: left; padding: 8px; background: #f8fafc; color: #64748b; font-weight: 600; font-size: 10px; text-transform: uppercase; }
        .tier-table th:nth-child(2), .tier-table th:nth-child(3) { text-align: right; }
        .tier-table td { padding: 8px; border-bottom: 1px solid #f1f5f9; }
        .tier-table td:nth-child(2), .tier-table td:nth-child(3) { text-align: right; font-family: 'Consolas', monospace; }
        .tier-name { font-weight: 600; color: #334155; }
        .tier-range { font-size: 9px; color: #94a3b8; margin-left: 4px; }
        .tier-price { color: #64748b; font-size: 10px; margin-left: 6px; }
        .tier-total-row { background: #f8fafc; font-weight: 700; }
        .tier-total-row td { border-bottom: none; padding: 10px 8px; }
        .loss-row { background: #fffbeb; }
        .loss-row td { color: #b45309; font-weight: 600; }
        .footer { text-align: center; padding-top: 16px; border-top: 1px solid #e2e8f0; margin-top: 20px; }
        .footer-text { font-size: 10px; color: #94a3b8; }
        .footer-date { font-size: 9px; color: #cbd5e1; margin-top: 4px; }
      ...
      <div class="households-grid">
        <div class="household-card tret-card">
          ...
          <div class="household-details">
            <div class="detail-row">
              ...
              <span class="detail-value tret-value">${formatKwhValue(kwhTret)} kWh</span>
            </div>
            <div class="detail-row">
        ...
        <div class="household-card lau-card">
          ...
          <div class="household-details">
            <div class="detail-row">
              <span class="detail-label">Số điện:</span>
              <span class="detail-value lau-value">${formatKwhValue(kwhLau)} kWh</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">Tỷ lệ:</span>
    ...
    `;
  }, [month, totalAmount, totalKwh, kwhTret, kwhLau, results]);
...
✓ Đối soát chênh lệch: 0 đ (Khớp 100%)`;
  ...
  return (
    <div ref={containerRef} className="w-full max-w-md mx-auto min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between font-sans selection:bg-teal-500 selection:text-slate-950 shadow-2xl border-x border-slate-900 pb-4 relative overflow-x-hidden">
      ...
      <main className="flex-1 px-3 py-2 space-y-2">
        ...
        <div className="grid grid-cols-5 gap-1">
          ...
          <ZaloNotifyButton
            ...
            kwhTret={kwhTret}
            kwhLau={kwhLau}
            tretAmount={results.households[0].allocatedTotal}
            lauAmount={results.households[1].allocatedTotal}
            tretAvgPrice={results.households[0].averagePricePerKwh}
            lauAvgPrice={results.households[1].averagePricePerKwh}
            lossKwh={results.lossKwh}
          />
          ...
          <button
            ...
            className="flex flex-col items-center justify-center p-1 rounded-lg bg-slate-900 border border-slate-800 hover:bg-slate-800 active:scale-[0.97] transition-all text-slate-400"
          >
            <span className="text-base mb-0.5">📋</span>
            <span className="text-[7px] font-bold">Copy</span>
          </button>
          ...
          <button
            onClick={() => {
              ...
              if (!pdfContent || !pdfContent.innerHTML.trim()) {
                triggerToast("Đang chuẩn bị dữ liệu...");
                return;
              }
              ...
              if (printWindow) {
                printWindow.document.write(`
                  ...
                  <head>
                    <meta charset="UTF-8">
                    <title>Hóa Đơn Tiền Điện - ${formatDateDisplay(month)}</title>
                  </head>
                ...
                triggerToast("Không thể mở cửa sổ in.");
              }
            }}
            className="flex flex-col items-center justify-center p-1 rounded-lg bg-slate-900 border border-slate-800 hover:bg-slate-800 active:scale-[0.97] transition-all text-indigo-400"
          >
            <span className="text-base mb-0.5">📄</span>
            <span className="text-[7px] font-bold">In PDF</span>
          </button>
          ...
          <button
            ...
            disabled={savingBill}
            className="flex flex-col items-center justify-center p-1 rounded-lg bg-slate-900 border border-slate-800 hover:bg-slate-800 active:scale-[0.97] transition-all text-teal-400 disabled:opacity-50"
          >
            <span className="text-base mb-0.5">{savingBill ? "⏳" : "💾"}</span>
            <span className="text-[7px] font-bold">{savingBill ? "Lưu..." : "Lưu"}</span>
          </button>
        </div>
            ...
            {chartData.length > 0 ? (
              <>
                ...
                <AIInsights
                  ...
                  historicalBills={savedBills as any}
                />
      ...
      {/* Hidden PDF Content for Print (off-screen) */}
      <div
        id="pdf-content"
        style={{ position: "fixed", left: "-9999px", top: "0", width: "600px", zIndex: -1 }}
